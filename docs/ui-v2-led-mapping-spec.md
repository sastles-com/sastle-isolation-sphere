...
### 5.2b 表示ルール — 前面/裏面の描き分け (2026-07-06 仕様追加)

live モードでは「実機を外から見ている」見え方に寄せる:

1. **前面の LED のみ映像 RGB を適用**する。**裏面 (法線がカメラと逆向き) は 50% グレー
   (`#808080`) 固定** (brightness 乗算しない)
2. **ワイヤーフレーム球は live 中は非表示** (opacity 0)。定数 1 つで「薄表示 (0.05 程度)」に
   切替できるようにしておく。params / strip モードでは従来どおり 0.12
3. **前面判定**: LED の単位球面座標 = 法線。カメラは +Z 固定 ([0,0,6]) なので、
   **表示用回転 (グループに適用している非共役 quaternion `q`) で回した法線の z 成分**
   `nz = (R(q)·p).z` を使い、`nz > 0` → 前面 (映像色) / `nz ≤ 0` → 裏面 (グレー)

> ⚠️ **回転の使い分けに注意**: サンプリング (§5.2) は**共役** quaternion、
> 前面判定は**非共役** (表示と同じ) quaternion。サンプリング用に回した座標を
> 流用すると判定が逆になる。800 点 × 回転 2 回でも計算量は問題ない
> (判定は z 成分しか使わないので、回転行列の 3 行目相当だけ計算すれば半分以下になる — 任意)

4. **ブレンディング変更**: live モードは `AdditiveBlending` をやめ
   **`NormalBlending` + `depthWrite: true`** にする。理由: 加算合成だと裏面グレーが
   前面の色に加算されて全体が白く濁る。Normal + depthWrite なら手前の LED が
   奥の LED を正しく隠す。params / strip モードは従来どおり Additive のまま
...
- `'live'`: vertexColors 使用、material color=#fff、brightness は頂点色に焼き込み済みのため
  opacity は固定 (1.0)。ブレンディング・球メッシュの扱いは **§5.2b の表示ルールに従う**
  (NormalBlending + depthWrite、前面のみ映像色 / 裏面 50% グレー、ワイヤーフレーム非表示)
...
2. 実機を手で回転させると、UI 上で点群 (球) が回る一方、**映像の模様は世界固定に見える**
   — 実機の見た目と同じ挙動
3. **前面 (法線がカメラ向き) の LED だけが映像色**になり、**裏面の LED は 50% グレー**で
   表示される。回転すると前面/裏面の境界が球の輪郭 (シルエットエッジ) に沿って移動する
4. live 中は**ワイヤーフレーム球が見えない** (定数変更で薄表示に切替できる)。
   裏面グレーが前面の色に混ざって白く濁らない (NormalBlending + depthWrite の確認)
5. 再生停止 → 3 秒で params 着色へフォールバック、再生再開で live に自動復帰
   (params / strip モードでは従来どおり Additive + ワイヤーフレーム 0.12)
6. FRAME_PREVIEW 配信中も WS の STATE_UPDATE / コマンドが遅延しない
7. スマホ実機で 60fps 維持、バックグラウンドタブで CPU を食わない
