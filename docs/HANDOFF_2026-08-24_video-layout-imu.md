...
**追記 (2026-08-25): 解決。** 2台は別ユニットで、id を以下に確定した。
config.json は `spheres[]` 配列になり、**ファームが自機 MAC で自分のエントリを
選ぶ**ようになったため、core ごとに config を書き換える必要はなくなった
(`core/src/ConfigManager.cpp: _resolveSphere`)。

| id | MAC | static_ip | 備考 |
|----|-----|-----------|------|
| sphere001 | `F0:9E:9E:32:67:D0` | 192.168.49.101 | このハンドオフ時点で「2台目」と呼んでいたユニット。現行機 |
| sphere002 | `AC:27:6E:D1:93:7C` | 192.168.49.102 | 2026-08-24 まで開発に使っていたユニット |

IMU はどちらも BNO055 (`atoms3r_ota` で両方ビルド可)。同一 IP / 同一 MQTT
clientId による「原因不明の瞬断」は、MAC で id と static_ip が分かれるため
2台同時電源ONでも起きない。
